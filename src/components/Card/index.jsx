...
export default function Card({isActive, filmChoose, start, loadingButton, save, saveButton}) {
  ...
  const trailer = filmChoose?.trailer;
            ...
            {isActive ? (
                <div class={"flex py-5 justify-center items-center flex-col"}>
                  ...
                  <div class={"flex items-center justify-center gap-4 my-3"}>
                    ...
                    <span class={"flex items-center justify-center gap-1 text-accent bg-gray-100/70 border-neutral-300 border px-2 text-[12px] font-bold"}> <IoStarSharp/> {Math.round(film.vote_average)} / 10 </span>
                    <span class={" border-neutral-300 border text-gray-300 px-2 text-[12px]"}>{film.runtime} min</span>
                  </div>

                  <h2 class={"italic font-medium text-lg hover:text-accent"}> {film.title} </h2>
                     ...
                     <div class={"my-2 text-sm h-18"}>
                       <p class={"text-gray-600"}>Esse filme não possui nenhuma sinopse</p>
                        ...
                        <div class={"my-2 text-sm h-18 overflow-y-scroll"}>
                            <p class={"text-gray-600"}>{film.overview}</p>
                        </div>   
                    )
                  }
