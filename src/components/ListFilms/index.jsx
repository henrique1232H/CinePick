...
export default function ListFilm({ listForFilms, setListForFilm, save, saveButton }) {
  ...
  const [buttons] = useState([
    { name: "TODOS", isActive: true, id: 0 },
    { name: "PARA ASSISTIR", isActive: false, id: 1 },
    { name: "ASSISTIDOS", isActive: false, id: 2 },
  ]);
    ...
    <div>
      <div>
        ...
        <p className={"font-sans text-[12px] text-gray-500 mt-1"}>
          Seus filmes salvos para assistir ( {filmWatched.length} de {listForFilms.length} assistidos ).
        </p>
      </div>
      ...
      <div className={"flex my-3 p-1 gap-4 font-sans border border-gray-300 text-[12px] text-gray-500 font-semibold"}>
        {buttons.map((button) => {
          return (
            <button
              key={button.id}
              className={`${button.isActive ? "bg-ink text-white" : "bg-none"} transition-all p-2 cursor-pointer`}
              onClick={() => {
                if (!button.isActive) {
            ...
            >
              {button.name} ( {button.id !== 2 ? listForFilms.length : filmWatched.length} )
            </button>
          );
        })}
      </div>
            ...
            return (
              <SalvFilmCard
                ...
                markDone={() => {
                  ...
                  setFilmWatched(listForFilms.filter((film) => film.status !== true));
                }}
              >
                <input
                  type="text"
